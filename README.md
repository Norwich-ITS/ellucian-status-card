...
An Ellucian Experience extension that surfaces system status from Atlassian Statuspage-powered status pages. No pipeline, microservice, or API key is required — the cards fetch the public Statuspage API directly from the browser (Statuspage serves open CORS headers).

## Cards

This extension ships two card types, both rendered by the same component (`src/cards/EllucianStatus.jsx`):

- **Ellucian System Status** — pinned to Ellucian's status page. Shows the overall status badge, active incidents (impact, current status, latest update from Ellucian engineers, link to details), an Affected Systems list of any non-operational components, and active/upcoming maintenance windows with the systems they affect.
- **Statuspage System Status** — the same card, generic. Point it at any Statuspage-powered vendor (Canvas/Instructure, Zoom, Cloudflare, GitHub, etc.) via its configuration. Defaults to Ellucian's status page when left blank. Note: this only works with Atlassian Statuspage (statuspage.io) sources — status.io, Microsoft 365, and Google Workspace use different, incompatible status APIs.

## Card configuration

Both fields are optional; blank values use the defaults:

- **Status API Base URL** — defaults to `https://status.elluciancloud.com/api/v2`. For another vendor, enter the status page's address in any reasonable form — vanity domain (`status.example.edu`) or direct statuspage.io address (`example-university.statuspage.io`). The card normalizes the value automatically: `https://` is added if missing, `http://` is upgraded to `https://`, trailing slashes are trimmed, and `/api/v2` is appended if absent.
- **Refresh Interval (minutes, 1-60)** — defaults to 5. Values outside 1-60 are clamped.

## Deploying to your own Experience instance

1. Run `nvm use` and `npm install`.
2. Set `publisher` in [extension.js](extension.js) to your institution.
3. Copy [sample.env](sample.env) to `.env` and add your Experience extension upload token and target environment.
4. Run `npm run deploy-dev` (or `deploy-prod`).
5. In Experience Setup, enable the extension and associate it with your environment. In Card Management, assign roles to the card(s) — configuration can be left blank for the Ellucian card.

---

The sections below are the standard Ellucian extension SDK notes generated with the project.
